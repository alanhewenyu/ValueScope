...
若反过来先更新份额再定价：涨幅被摊薄成 +1.8% —— 新钱凭空分走老钱的收益，错。`}</Code>
      ...
      <P><strong className="text-gray-800 dark:text-gray-200">③ 个股 YTD 归因</strong>：每只票按（年份 × 券商）存年初基准（价格/数量/成本；年中建仓以建仓成本为基准），逐股算本币价差后加总。持有部分——摊薄口径 = 当前浮盈 − 年初已有浮盈（摊薄成本吸收了已实现收益，跨加减仓精确）；平均口径部分卖出后 = (现价 − 年初价) × 剩余数量。卖出部分在成交瞬间锁定 (卖价 − 年初基准价) × 数量，之后不再变。不含现金利息与汇率路径，所以与 ①② 有差是正常的。
      </P>
      ...
      <Tbl
        ...
        rows={[
          ...
          ["股息 / 利息到账", "现金/杠杆 → 直接改余额", "别记成入金；摊薄口径券商若下调持仓成本，照抄券商新值"],
      ...
      <H3>Q: 分红后券商把持仓成本调低了，要跟着改吗？</H3>
      <P>
        跟着改——<strong className="text-gray-800 dark:text-gray-200">分红后照抄券商屏幕上的成本，永远是正确操作</strong>，两种成本口径都适用。摊薄口径（国内券商/富途）分红必调减成本；盈透对港股等市场的分配实务上也常直接下调 Average Price（不少港股分红从股份溢价账派发，属资本性分配），美股普通股息则通常记为收入、成本不动——以你券商屏幕的实际显示为准。归因上两种情形都自洽：券商调了成本时，股价降 1 元、成本降 1 元，浮盈和 YTD 归因不变，股息只体现在净值层面；券商没调时，个股归因显示除权那一跌、股息落在净值层面，分红股在归因表里显得偏弱是口径特性，不是错。唯一不变的纪律：到账现金要记（第五节），成本照抄券商。
      </P>
