      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        {kpis.map((k) => (
          <div key={k.label} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4">
            ...
            <div className={`text-lg font-bold ${k.green ? "text-red-600 dark:text-red-400" : "text-gray-900 dark:text-white"}`}>{k.value}</div>
          ...
          <tbody>
            {SAMPLE_HOLDINGS.map((h) => (
              <tr key={h.ticker} className="border-b border-gray-50 dark:border-gray-800/50">
                ...
                <td className={`px-4 py-2.5 text-right tabular-nums font-medium ${h.pnl >= 0 ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400"}`}>
