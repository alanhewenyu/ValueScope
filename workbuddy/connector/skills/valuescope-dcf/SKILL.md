...
**禁止**：手算 DCF、自己去网上找 PE/PB 倍数（引擎有 `get_relative_valuation`）、
把网页搜到的目标价或机构评级当作估值结果交付、跳过 `run_dcf` 的第一次调用。
...
三个工具，分工明确——**先用秒回的两个定位，再决定要不要做深水的那个**：

| 工具 | 回答什么 | 需要假设参数 | 耗时 |
|---|---|---|---|
| `get_score` | 这家公司整体怎么样 | 否 | 秒回 |
| `get_relative_valuation` | 相对自己的历史贵不贵 | 否 | 秒回 |
| `run_dcf` | 它到底值多少钱 | 是（第二步） | 需联网研究，数分钟 |

**相对指标与内在价值必须互相验证。** 若 DCF 显示低估、而倍数处在历史高位，
这个矛盾必须解释清楚，不能只报对自己有利的那个。

### `get_score` — 四维综合评分

`get_score(ticker)` → 估值 / 质量 / 成长 / 动量，各 0-100 独立打分后等权合成总分。

- **估值**：PE 历史分位 40% · Forward PE 30% · 经营 PE 30%
- **质量**：ROE 水平与趋势 · 收入与利润率趋势 · 杠杆 · Piotroski F 分 · 利润质量
- **成长**：预期 EPS 增速 40% · 3 年收入 CAGR 30% · 最新收入增速 30%
- **动量**：12-1 月收益 50% · 6-1 月收益 50%（剔除最近 1 个月）

**呈现要拆开四维讲**——总分接近的两家公司构成可能完全不同，要指出哪一维拖累、
哪一维支撑。缺失数据按比例重新加权，不计罚分。

⚠️ 估值维度用的是 **PE 历史分位，不是 DCF**。"值多少钱"请用 `run_dcf`。

### `get_relative_valuation` — 当前倍数 + 历史分位

`get_relative_valuation(ticker, years=5)` → 当前 P/E、Forward P/E、P/B、P/S、
EV/EBITDA，以及 P/E 与 P/B 的历史区间和当前分位。`years` 可取 1-10。

**分位只与公司自身历史比较**，不含可比公司横向对比。低分位不等于便宜——基本面
恶化时倍数下移是合理的；高分位也不等于贵——成长加速时市场愿意给更高倍数。
判断行业整体重估时，仍需结合同业倍数。
