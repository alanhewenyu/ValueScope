...
`get_score`(四维体检)和 `get_relative_valuation`(当前倍数 + 历史分位)**一次调用秒回**,
回答"这家公司怎么样、相对自己的历史贵不贵"。

`run_dcf` 回答"它值多少钱",模仿股权分析师的工作流,由调用方模型扮演分析师,分两步:

1. **分析材料** —— 不带任何假设调用 `run_dcf(ticker)`,返回历史财务数据、每个参数的历史区间、引擎算出的 WACC 与历史平均税率,以及一份告诉模型如何评估每个假设的分析指南。**此步不返回估值**——先给一个数字只会锚定模型接下来的判断。
2. **最终** —— 模型联网搜索业绩指引与一致预期,推理每个参数,再带上假设调用 `run_dcf(ticker, <参数>)` 得到最终估值:每股内在价值、价值桥、逐年预测表、敏感性矩阵、反向 DCF(市价隐含了什么假设)。

同时暴露一个 `dcf` MCP prompt,在支持 MCP prompt 的客户端里呈现为一键工作流(材料 → 搜索 → 推理 → 三情景)。
